...
<tiles:insert definition="template.default" flush="true">
	...
	<tiles:put name="body" type="string">
		<script>
			...
			{
				document.acreditacionAdminEventoForm.radio[0].checked = true;
			}
		
			function detalleEvento(evento){
				document.location.href = "AcreditacionAdminEvento.do?method=onListarDetalleEvento&evento="+evento;
			}
			function enviarNotificacion(evento){
				document.location.href = "AcreditacionAdminEvento.do?method=onEnviarMails&evento="+evento;
			}
			
			function cambiarEstadoEvento(estado){
				 var i;
				 var sel;
				    for (i=0;i<document.acreditacionAdminEventoForm.radio.length;i++){
				       if (document.acreditacionAdminEventoForm.radio[i].checked)
				       {
				    	   sel = document.acreditacionAdminEventoForm.radio[i].value;
				    	   break;
				       }
				         
				    }

				    if(i<=document.acreditacionAdminEventoForm.radio.length)
				    {document.location.href = "AcreditacionAdminEvento.do?method=onCambiarEstadoEvento&evento="+sel+"&estado="+estado;}
				    else
				    {
		...
		<div align="center"><html:form action="/AcreditacionAdminEvento">
			...
			<display:table excludedParams="*"
				requestURI="AcreditacionAdminEvento.do?method=onListar" pagesize="10"
				id="evento" name="acreditacionAdminEventoForm.eventos"
